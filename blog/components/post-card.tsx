...
export default function PostCard({
  ...
  if (variant === "list") {
    return (
      ...
      >
        <Link
          href={`/blog/${slug}`}
          className="block group border-b border-black/5 dark:border-white/5 pb-8 mb-8 last:border-0 last:mb-0 last:pb-0 hover:bg-black/[0.02] dark:hover:bg-white/[0.02] transition-colors p-4 -mx-4 rounded-2xl"
        >
          <article className="flex items-center gap-6 sm:gap-10 transition-transform duration-300 group-hover:-translate-y-1">
            <div className="w-16 sm:w-20 shrink-0">
              <span className="text-xl sm:text-2xl font-display font-bold text-gray-400 dark:text-gray-500 group-hover:text-black dark:group-hover:text-white transition-colors">
                {dateStr.split(",")[0]}
              </span>
            </div>
            ...
            {coverImage ? (
              ...
              <div className="w-24 h-24 sm:w-32 sm:h-32 shrink-0 overflow-hidden rounded-xl relative bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 flex items-center justify-center group-hover:border-black/20 dark:group-hover:border-white/20 group-hover:shadow-[0_8px_30px_rgba(0,0,0,0.12)] dark:group-hover:shadow-[0_8px_30px_rgba(255,255,255,0.1)] transition-all duration-500">
                <span className="text-4xl text-gray-400 dark:text-gray-500 font-display font-bold">
                  {title?.charAt(0) || "U"}
                </span>
                {rank && (
                  <div className="absolute top-2 left-2 w-8 h-8 rounded-full bg-white/30 dark:bg-black/40 backdrop-blur-md border border-white/40 dark:border-white/20 flex items-center justify-center text-sm font-display font-black shadow-lg text-black dark:text-white z-10">
  ...
  const formattedDate =
    month && day ? `${month}.${day}` : dateStr.split(",")[0];
    ...
    >
      <Link href={`/blog/${slug}`} className="block">
        <article className="h-full flex flex-col pt-6 border-t border-black/10 dark:border-white/10 group-hover:border-black/40 dark:group-hover:border-white/40 transition-colors duration-500">
          ...
          {coverImage ? (
            <div
              className={`${aspectClass} w-full relative overflow-hidden bg-black/10 dark:bg-black/50 group-hover:-translate-y-2 group-hover:shadow-[0_20px_40px_rgba(0,0,0,0.15)] dark:group-hover:shadow-[0_20px_40px_rgba(255,255,255,0.08)] transition-all duration-500 mt-2 rounded-sm`}
            >
              {isImageLoading && (
                <div className="absolute inset-0 bg-black/10 dark:bg-white/10 animate-pulse z-0" />
              )}
            ...
            <div
              className={`${aspectClass} w-full relative bg-black/5 dark:bg-white/5 flex items-center justify-center group-hover:-translate-y-2 group-hover:shadow-[0_20px_40px_rgba(0,0,0,0.15)] dark:group-hover:shadow-[0_20px_40px_rgba(255,255,255,0.08)] transition-all duration-500 mt-2 rounded-sm`}
            >
