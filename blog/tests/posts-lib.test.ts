...
describe("getPostSlugs", () => {
  ...
  test("returns extensionless slugs", () => {
    const slugs = getPostSlugs();
    for (const slug of slugs) {
      assert.ok(
        !slug.endsWith(".md"),
        `Expected "${slug}" to be extensionless`,
      );
    }
  });
...
describe("getPostBySlug", () => {
  ...
  test("calculates readTime", () => {
    ...
    assert.ok(post !== null);
    assert.ok(
      post.readTime?.endsWith("min"),
      `Expected readTime to end with "min", got "${post.readTime}"`,
    );
  });
});
  ...
  test("excludes posts with publish: false", () => {
    ...
    for (const post of posts) {
      assert.notEqual(
        post.publish,
        false,
        `Post "${post.title}" has publish=false but was included`,
      );
    }
  });
...
describe("getAllTags", () => {
  test("returns sorted unique tags", () => {
    ...
    for (let i = 1; i < tags.length; i++) {
      assert.ok(
        tags[i - 1] <= tags[i],
        `Tags not sorted: "${tags[i - 1]}" > "${tags[i]}"`,
      );
    }
...
describe("getRelatedPosts", () => {
  test("does not include the original post", () => {
    ...
    assert.ok(
      !slugs.includes(posts[0].slug),
      "Related posts should not include the original",
    );
  });
